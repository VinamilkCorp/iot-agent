// ── Dashboard (uses shared ui-utils.js loaded via <script> before this) ─────
...
// Tải cổng serial qua HTTP (dashboard chạy ngoài Electron)
...
// ── Monitor: lịch sử cân nặng & biểu đồ ───────────────────────────────────
const history = [];
let currentUnit = "";
...
function drawChart() {
  ...
  pts.forEach((p) => {
    ctx.beginPath(); ctx.arc(xOf(p.t.getTime()), yOf(p.w), 3, 0, Math.PI * 2); ctx.fill();
  });
